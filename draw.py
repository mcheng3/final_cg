...
def add_mesh(polygons, mesh_filename):
    vertices = []
    faces = []
    with open(mesh_filename) as mesh:
        for line in mesh:
            args = line.split()
            if args:    
                if args[0] == 'v':
                    #If the line begins with 'v', add the coordinates to vertices
                    vertices.append(map(float, args[1:]))
                if args[0] == 'f':
                    #if the line beings with 'f', add the vertex number (minus 1 because it's 1-indexed), and ignore anything that comes after a '/'
                    faces.append(map(lambda i: int(i[:i.find('/')]) - 1 if '/' in i else int(i) - 1, args[1:]))
    #Rescale to fit 500x500
    max_coord = max(map(max, vertices))
    min_coord = min(map(min, vertices))
    vertices = [map(lambda i: ((i - min_coord)*500/(max_coord - min_coord)-250)*.9+250, vertex) for vertex in vertices]
    #Add polygons, works with any non-complex polygon
    for face in faces:
        c = 0
        while(c < len(face) - 2):
            add_polygon(polygons, vertices[face[0]][0], vertices[face[0]][1], vertices[face[0]][2]
                                , vertices[face[1+c]][0], vertices[face[1+c]][1], vertices[face[1+c]][2]
                                , vertices[face[2+c]][0], vertices[face[2+c]][1], vertices[face[2+c]][2])
            c += 1
